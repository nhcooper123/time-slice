...
Time bins (equal sized)
-----------------------------------

What do we want to vary? Metrics? Number of bins?

Note we might want to exclude node but current won't work due to mismatch - ooh maybe use clean.data to fix??? Yes that works

no.nodes <- clean.data(morphospace, tree)
m<- no.nodes$data         
          

```{r, eval = FALSE}
time.bin.sim <- function(times, metric) {

  sub <- time.subsamples(data = morphospace, tree = tree, method = "discrete",
                time = times, FADLAD = FADLAD, inc.nodes = TRUE)
                
  disparity <- summary(dispRity(sub, metric = metric),
                      round = 10)
}
```                      
...
What do we want to vary? Metrics? Number of slices? Model of evolution

Note we might want to exclude node but current won't work due to mismatch - ooh maybe use clean.data to fix???


```{r, eval = FALSE}
time.slice.sim <- function(times, metric) {

  sub <- time.subsamples(data = morphospace, tree = tree, 
    method = "continuous", model = "gradual", time = times,
    FADLAD = FADLAD, inc.nodes = TRUE)
