...
One final issue is that to commute Gower distances we need all taxa to have some overlapping data. If not Gower distances will be NA (Gower distance does not impute missing data like some other methods). To avoid this problem we can remove the taxa with no overlaps as follows. This is commented out below because there are no non overlapping taxa in the Beck and Lee 2014 dataset. 
...
## Rewrite this to file for later use
## saveRDS(matrix_dist, file = paste0("../data/processed/distance_matrix.", slug ,".Rda"))
```
