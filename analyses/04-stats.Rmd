...
This script requires the outputs from `02-time-slice-analyses.Rmd`. The code below runs the analyses for the Beck and Lee 2014 dataset - change the slug name to run the others. This of course can all be written into functions but we ran out of time so here is some suboptimal but working code!
...
strat_min <- 
  strat %>%
  group_by(model) %>%
  filter(bs.median == min(bs.median, na.rm = TRUE))
...
all_peaks_max <- rbind(strat_max, duration_max, number_max, strat2_max, duration2_max, number2_max)
write.csv(all_peaks_max, file = paste0("../outputs/peaks.max.", slug, ".csv"), quote = FALSE, 
          row.names = FALSE)

all_peaks_min <- rbind(strat_min, duration_min, number_min, strat2_min, duration2_min, number2_min)
