...
We now only calculate the disparity for the non-rarefied subsets and the subsets rarefied to contain only 3 taxa (the lowest level of rarefaction).
...
## Principal colors
colors <- palette()[1:7][-1] #TG: that can be made fancier
colors[7] <- "black"
## Color combination (for the CI)
colors_CI <- c("pink", "lightgreen", "lightblue", "lightcyan", "lightpink", "beige", "lightgray")
## y limits
...
colors_CI <- c("pink", "lightgreen", "lightblue", "lightcyan", "lightpink", "beige", "lightgray")
## y limits
ylim <- range(unlist(lapply(out2$results, function(X) return(c(min(X[,5], na.rm = TRUE), max(X[,5],
  na.rm = TRUE))))))
...
models <- list("equalbins", "acctran", "deltran", "random", "proximity", "punctuated", "gradual")
...
## Adding the legend
legend(x = "bottomleft", legend = c("time bins", "acctran", "deltran", "random", "proximity"),
  col = colors[1:7], lty = 1, bg = "white")
