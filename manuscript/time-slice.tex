...
Disparity-through-time analyses are popular and routinely applied to palaeobiological datasets.

Although many methods exist to measure disparity, all more or less assessed (metrics, tests, etc...), only one method exist for looking at disparity through time: pooling data into time bins.

This may lead to some problems down the line as a methodological bottleneck (i.e. most disparity-through-time analysis are, in the end, based on pooling data into bins).


Here we propose a novel method to complement the disparity-through-time tool-kit: time-slicing.

This method allows to sample disparity through time at any fixed point in time rather than binning data between two fixed points in time.

Additionally, this method allows to use all the phylogenetic data available (tips, nodes 



\textit{and} branches) and specify the implied model of evolution between each time-slice (punctuated or gradual).


We test this method on four example datasets and compared how it performs at common disparity-through-time analysis: the effect of mass extinction and the time at the maximal and minimal disparity.

We found that the sampling effect has a remarkable effect on our interpretation of disparity-through-time analysis.

Using multiple methods for assessing disparity-through-time (rather than only one!), we can have a better picture of what is happening.
...
This can be dealt with using rarefaction methods, i.e. repeating the analysis while re-sampling the taxa to have the same amount of taxa in each bin (e.g. using bootstrapping with limited resampling).
...
%TG: well done! Nice description of the method! I figured out it was pretty obvious in our (my) head but it's actually more painfull to write down.
...
This is also true for stratigraphic time bins in the \cite{bapst2016topology} and \cite{wright2017bayesian} datasets, although when using equal time bins the peaks are later than the time slicing methods, or very similar (Figure \ref{figure:peak1}; Supporting Information Appendix S2: Figure AX). %appendix
...
% Plots showing the differences
  ...
  %TG: Would it be possible to make the bins points into a thick line? Though that's just aesthetic...
...
%TG: ideas for the discussion:
  % Extinction test: in Brusatte's dataset, ther seems to be an effect of the model (gradual vs. punctuated) on recovering the effect of the extinction or not when using stratigraphy: not significant for bins and punctuated slices but signif for gradual slices. TG: kind of showing why it's cool to allow different models of evolution.
  % peak test: the peak really depends on which time slicing method is used! This is probably expected for the disparity metric (different metrics give different results) but was never acknowledge to be the case for different methods of time subsampling! Kind of cool too!
