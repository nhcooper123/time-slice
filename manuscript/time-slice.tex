...
% NC: Nah it's just a group and makes it clearer here that I'm actually doing some programming work. Takes about 30s to transfer and just means you/I need to change our remotes which again takes 30s.
%TG: OK then.
...
Disparity changes through time for each of the four datasets (Figure \ref{figure:dtt}, Figure in appendix, full tables in the appendix). % add details of appendix
...
\paragraph{Disparity peaks}
In the \cite{beckancient2014} and \cite{brusatte2014gradual} datasets, disparity peaks occur much earlier when time slicing rather than time binning approaches are used (Figure \ref{figure:peak1}, Appendix figure). % appendix details
This is also true for straigraphic time bins in the \cite{bapst2016topology} and \cite{wright2017bayesian} datasets, althought when using equal time bins the peaks are later than the time slicing methods, or very similar (Figure \ref{figure:peak1}, Appendix figure).
Across the three time binning methods, the \cite{beckancient2014} and \cite{brusatte2014gradual} datasets have similar disparity peaks whichever method is used, whereas in the \cite{bapst2016topology} and \cite{wright2017bayesian} datasets, stratigraphic (unequal) versus equally sized time bins make a large difference to where the disparity peak occurs (Figure \ref{figure:peak1}, Appendix figure).
...
\paragraph{Effects of mass extinction events.}
Mass extinction events influence disparity in all four datasets (Figure \ref{figure:extinction1}, appendix). 
However, whether this change in disparity is significant or not depends on the dataset, and on the method used to create time sub-samples (Figure \ref{figure:extinction1}, appendix). % appendix!!!
