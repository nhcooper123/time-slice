...
Results from disparity-through-time studies also provide insights into the ecological impacts of mass extinctions, competitive replacements, and the evolution of morphological innovations \citep{Brusatte12092008,Foote29111996,friedmanexplosive2010}.
...
Other studies split large time bins so they are of roughly equal size, but this is quite an \textit{ad hoc} procedure that can introduce more bias depending on where bins are split.
...
If there are insufficient taxa in a time bin, disparity cannot be calculated, so further analysis, e.g. correlations of disparity with climate proxies or other hypothesised drivers of morphological evolution, are not possible.
...
Here we present the time slicing methods using four datasets taken from the literature.
...
This results in even-sampling of the cladisto-space across time and while permitting to use an underlying model of character evolution (punctuated or gradual). % and others ADD). %TG: I don't have any other ones in stock. But it could be easy to come up with new ones. They're easy to implement.
