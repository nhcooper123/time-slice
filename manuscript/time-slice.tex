...
These datasets are freely available with their accompanying papers (Table \ref{datasets}), but for reproducibility purposes we also provide the data we used on the NHM Data Portal (CITE).
  ...
  %   {Dataset captions}
    ...
    \label{tab:data}  
  % \end{table}
% remove notes, add strat ages, add refs to dat sources
...
\begin{enumerate}
    ...
    This results in the bootstrapped disparity at a specific slice to effectively incorporate both the data from the descendant and the ancestor. %TG: now that I'm mentioning that, I should maybe rename this model. Lets discuss that on monday.
