...
We also recorded the number of taxa (or taxa and nodes for time-slicing methods) in each sub-sample as a proxy for taxonomic diversity (Appendix ??? \ref{tab:sub-samples}). % appendix tables maybe?
...
Disparity changes through time for each of the four datasets (F{}igure \ref{figure:dtt}, Supporting Information Appendix S2: Figure AX and Tables AX-X). % add details of appendix
