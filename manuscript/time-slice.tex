...
Whether the disparity represents an average across the interval (with no interpretation on if or how it varied within the time bin) or it is \textit{effectively} postulated to be constant, when analysing the changes in disparity through time, this method will only allow changes in disparity (in average or constant) to occur only \textit{between} intervals rather than also allowing for gradual changes \textit{within} intervals \citep[a pattern that is fairly common in the fossil record;][]{Hunt21042015}.
...
Finally, the difference between the time-binning and time-slicing methods could also simply be favoured towards time binning due to the nature of the fossil record itself.
...
It is important to note however, that, as specified above, the time slicing method also includes ancestral estimations data (whether through the nodes or the branches) that are by definition not available in the fossil record and thus are not influenced by its nature.
