...
The reviewer does indeed understands the \texttt{equal.splits} and \texttt{gradual.splits} models correctly: they selects the values in the matrix from the edge's descendant or ancestor with a probability value for each.
...
In fact, doing so pre-ordination can simply change the ordinated space geometry while doing in post-ordination leads to incorrect estimation of the space: (1) the number of elements changed hence changing parameters for future statistical test (e.g. degrees of freedom) and (2) the variance per axis will change as well.
On the other hand, our methods allows to approach a pure gradual model without modifying the space structure and properties.
...
\textit{For example, using the ``equal.split'' model between an ancestor and a descendant with PCO1 values of respectively $0.04$ and $0.03$, after a sufficient number of bootstrap replicates (e.g. 100) the value along the branch will be close to $0.5\times0.04 + 0.5\times0.03 = 0.035$.
By estimating this value rather than generating it (i.e. creating a new element midway along the branch that would be the average of the descendant and ancestor - $0.035$) we obtain the same results without modifying the morphospace properties.} lines @@@
...
We apologise for the confusion in the description of figure 3 and fixed it (see Reviewer 1, comment \ref{fig3_feckup}).
