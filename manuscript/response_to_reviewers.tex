...
Below we provide their comments in blue text with our responses below.
...
\textit{This approach is similar to Brusatte et al. (2011) but uses model based estimations (rather than parsimony) allowing us to control for ambiguous (i.e. poorly estimated) nodes.} lines 132-134.
...
Although empirical morphospaces from discrete or continuous data have been shown to have similar properties (CITATIONS), our morphospaces are based on discrete morphological data (originally collected for phylogenetic analysis;} c.f. \textit{geometric morphometric data) and include some phylogenetic information (see above).
Mathematically, our morphospaces are $n$ dimensional objects that summarise the distances between discrete morphological characters of the taxa present and their ancestors.} lines 138-147.
...
Across the three time binning methods, the Brusatte et al. (2014) dataset has similar disparity peaks whichever method is used, the Wright (2017) dataset only had variation in peaks when using unequal time bins (stratigraphy), whereas in the Bapst et al. (2016) and Beck \& Lee (2014) datasets, stratigraphic (unequal) versus equally sized time bins make a large difference to where the disparity peak occurs (Fig. 4; Supporting Information Appendix S2: Figs A3-A4).} lines 406-417.

We've also added a note about discrepancies in the time-slicing models in Beck \& Lee (2014): 

\textit{Additionally, there seems to be small discrepancies within the time-slicing methods (gradual.split vs. proximity) except in the Beck \& Lee (2014) dataset where the gradual split model recovered disparity peaks at younger ages than the proximity model (Fig. 3; Supporting Information Appendix S2: Figs A3-A4).} lines 417-421.
...
By estimating this value rather than generating it (i.e. creating a new element mid-way along the branch that would be the average of the descendant and ancestor - $0.035$) we obtain the same results without modifying the morphospace properties.} lines 290-296.
...
Additionally, this effect is likely to be most obvious in groups where the fossil record is ``patchy'' e.g. vertebrates, but less problematic for groups with a more continuous record like Foraminifera.} lines 473-484.
...
We thus suggest that both types of models (punctuated and gradual) are tested during analysis, unless there is strong independent support for one or the other.} lines 484-491.
...
We've added a figure describing the different time-sampling strategies which we hope makes this section clearer. 
...
\textit{We calculated the $n$$\times$$n$ distances using the Gower distance (CITATION), i.e. the number of mismatched characters over the number of shared characters.} lines 151-152.
...
Guillerme T., \& Cooper N. 2018. GitHub: nhcooper123/time-slice: Release for publication. Zenodo. http://doi.org/10.5281/zenodo.1172000.
...
We have fixed this. The paper accompanying the dispRity package is currently in review so hopefully we will have a better citation to provide at the proof stage. For now we have added the Zenodo doi and reference. We have changed the Lloyd 2015 reference to the reference for the the paper that introduces the package.
