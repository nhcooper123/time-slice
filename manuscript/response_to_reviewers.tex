...
% NC: What are these? I converted this to word already.
%TG: In Table 1, ages for Brusatte and Bapst should be respectively 168.5 - 66 and 207.2 - 66  (they are currently set to 0).
...
%TG: tone done the statement? % NC: yeah, not sure what else we can do. %TG: Does that kinda work?
...
We've add the following caveat to the discussion:

\textit{Finally, the difference between the time-binning and time-slicing methods could also simply be favoured towards time binning due to the nature of the fossil record itself.
The rates of sedimentation varies in time a space and can be really sporadic, influencing the groups found within and their temporal distribution (e.g. vertebrates, c.f. foraminifera).
In this case, different beds could represent different ``packages'' of fauna through time separated by gaps thus effectively binning the data rather than slicing.
For example, slicing through such strata will yield similar results whether where in time does the slice occur.
It is important to note however, that, as specified above, the time slicing method also includes ancestral estimations data (whether through the nodes or the branches) that are by definition not available in the fossil record and thus are not influenced by its nature.} lines @@@
...
%TG: Does that not sound a bit too defensive? We can just say we changed them no?
...
% NC: Just need to sort S2 when we have the new figure. I've added them as is to Dryad to get the reference link.
